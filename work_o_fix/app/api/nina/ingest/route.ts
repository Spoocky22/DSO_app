...
export async function POST(req: NextRequest) {
  ...
  // Le nom de fichier NINA est souvent la source la plus fiable pour le filtre,
  // surtout si l'évènement WebSocket renvoie un filtre vide/"?" et que l'agent
  // a appliqué un fallback L. On privilégie donc le fichier avant le champ brut.
  const filenameFilter = detectFilterFromText(filename)
  const normalizedFilter = normalizeFilterName(rawFilter)
  const targetFilter = detectFilterFromText(targetName)
  const filter = filenameFilter ?? normalizedFilter ?? targetFilter ?? "L"
  const filterFallback = !filenameFilter && !normalizedFilter && !targetFilter
